...
pygame.display.set_caption("Python Kruskal's Algorithm")
pygame.display.flip()
...
class Graph:
    ...
    def kruskalMST(self):
        ...
        # draw cost on screen
        costLabel_Surface = large_text.render("Cost: " + str(cost), True, RED)
        screen.blit(costLabel_Surface, (MSTX + 100, MSTY))

def drawNode(node):
    # draw node circle centered at (node.x, node.y)
    pygame.draw.circle(screen, BROWN, (node.x, node.y), nodeRadius)

    # draw the node number
    nodeSurface = large_text.render(str(node.nodeNum), True, WHITE)
...
def drawEdge(edge, color, edgeWidth):
    ...
    if weight !=0:
        weightLabel_Surface = large_text.render(str(weight), True, color)
...
def drawInstruction():
    ...
    instructOneLabel_Surface = medium_text.render('Step 1: Click Inside Box to Add Nodes', True, BLACK)
    screen.blit(instructOneLabel_Surface, (50, 460))

    instructTwoLabel_Surface = medium_text.render('Step 2: Enter Src Node #, Dst Node # and Weight, Then Click "Add Edges"', True, BLACK)
    screen.blit(instructTwoLabel_Surface, (50, 490))

    instructThreeLabel_Surface = medium_text.render('Step 3: Click "Find MST"', True, BLACK)
