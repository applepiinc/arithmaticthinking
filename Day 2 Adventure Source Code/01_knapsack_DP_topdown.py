...
# input items: choices
# input weight_limit: weight limit for each state
def zero_one_knapsack(items, weight_limit):
    ...
    if (weight_limit < items[-1].weight):
        ...
        value_without_current_item = zero_one_knapsack(items[0:-1], \
                                                       weight_limit)
...
# build two dimensional memo table
# because the problem state has two dimensions (capacity, max types of items)
